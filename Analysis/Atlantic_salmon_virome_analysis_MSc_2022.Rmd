...
library(ADImpute);packageVersion("ADImput") 
...
#vOTUs data
vOTUs <- read.csv("vOTU_nocontam.csv") %>% as.data.frame()
meta.design <- read.csv("Metadata.csv")
Vir_size <- read.csv("Contig_sizes.csv", sep = "\t")
tax <- read.csv("vOTU_tax_nocontam.csv") %>% as.data.frame()

#16s rRNA data
bacteria <- read.csv("Bozzi_OTUs.csv")
vOTUs.bac <- read.csv("vOTU_OTU_samples.csv") %>% select(!"vOTU_13036") #removal of contaminant
bac.tax <- read.csv("Bozzi_bacteria_taxonomy.csv")
bac.meta <- read.csv("Bozzi_bacteria_metadata.csv")
...
#pdf("Alpha diversity.pdf", height=4, width=6)
...
bac.tax <- read.csv("~/Desktop/Bozzi_et_al_2020_analysis/Davide_OTUs_Taxonomy.csv")
...
#pdf("Simpsons_corr.pdf", height=4, width=6)
